    ...
    switch (state) {
      ...
      case IN_SENTENCE:
        if (b == '.') {
          ...
          state = CHECKSUM1;  // Start of checksum, ignore the next 1-2 bytes, up to '\n' or '\r'
        } else if (b == 'N' && charIndex == 2) { // NMEA sentence starting with $GN
          b = 'P';  // We have $GN - change to $GP for compatibility with older software
          checkSum ^= b;
        } else {
          checkSum ^= b;
        }
